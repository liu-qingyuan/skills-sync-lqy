...
- **Prototype**：编写 UI 或逻辑代码来测试假设或探索设计空间。使用 `/prototype-zh` skill。创建原型作为资产。当关键问题是“它应该看起来如何”或“它应该如何表现”时使用。
- **Grilling**：与 agent 对话。使用 `/grilling-zh` 和 `/domain-modeling-zh` skills。一次问一个问题。默认使用此类型。
...
1. 运行 `/grilling-zh` + `/domain-modeling-zh` 会话，暴露未决决策。一次问一个问题。
...
1. 加载**整张决策图**作为上下文。
2. 运行会话来解决该 ticket，并按需调用 skills。如果不确定，使用 `/grilling-zh` 和 `/domain-modeling-zh`。
...
如果用户跳过它，建议直接实现，或使用 `/to-prd-zh` 安排多会话实现。
