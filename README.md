...
Pi worker 是默认选择。当前 Pi 会话提供 `run_ralph` 工具时，用户明确要求执行会直接调用该阻塞工具；只加载 skill 或请求计划不会启动循环。工具不可用时回退到 locked Pi CLI，Codex 和 Claude Code worker 仅在用户明确点名时使用。`PI_RUN_RALPH_WORKER=1` 标记子 Pi iteration，禁止它再次启动嵌套 Ralph。
