...
- provision 报告 dirty 时，producer agent 必须在 exact target worktree 中自行理解、完成、验证、commit 并正常 push 可确认的改动，再重试；不得把工作交给用户，也不得用 stash、reset、`git clean`、临时 clone 或其他 worktree 绕过 gate。只有改动意图或完成标准无法确定时才询问用户。
