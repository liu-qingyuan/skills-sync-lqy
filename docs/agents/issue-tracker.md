...
- 一个 branch 对应一个独立 `git worktree`，一个 worktree 同时只运行一个持有本地 OS 文件锁的 Ralph。
- 默认 branch 名是 spec 标题生成的纯 `<slug>`；若已存在则停止并要求显式指定，不自动复用或追加随机后缀。
- 默认 worktree 位于主 worktree 同级目录，名称为 `<repo-name>-<slug>`。
