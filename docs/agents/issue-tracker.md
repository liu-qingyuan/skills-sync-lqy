...
- 一个 spec 恰好绑定一个 branch；首个子 Ticket 进入 `ready-for-agent` 后冻结完整 `## Git`，父 spec 与同一 spec 的所有子 Tickets 必须继续与该 contract 完整一致。
- 未显式指定 `Branch` 时，使用 producer 的 `--repo` 所在 worktree 当前 attached branch；主 worktree 因此仍默认 `main`，已有 feature worktree 则保留其 exact branch。detached 时回退远程默认分支。不得自动生成 feature slug branch。
- 非主分支必须来自用户显式指定，或来自 producer 当前所在的已 attached worktree；不得因为发现其他 worktree 而自行选择 branch。
...
- 当前不支持跨 branch `Blocked by`。外部 blocker 必须有有效 Git contract 且 `Branch` exact match；同 branch 的不同 spec 可以使用不同 `Base branch` 或 `Base commit`。
...
- `to-spec-lqy` 在创建 Ralph-ready 父 spec 前完成 provision；后续 producer 复用同一 exact worktree。
- linked worktree 自动共享 Git config 和 ignore 规则；local-config allowlist 只复制 ignored/untracked 的 `.codex/config.toml` regular file，其他 ignored 内容、trust 和运行时 session 不复制。
- worktree dirty 时，producer agent 完成可确认的改动，验证并 commit/push 后重试。仅在意图不明时询问用户；禁止绕过 gate。
