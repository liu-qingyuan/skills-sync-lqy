...
架构审查在操作系统临时目录中呈现为单个独立的 HTML 文件。 Tailwind 和 Mermaid 都来自 CDN。 Mermaid 可靠地处理图形形状的图表；手工构建的 div 和内联 SVG 可以处理更多的编辑视觉效果（质量图、横截面）。将两者混合起来——不要一切都依赖美人鱼，它会开始看起来很普通。
...
图表很有分量。散文简洁、朴素，并且使用术语（来自“/codebase-design-zh”技能）而不拘礼节。
...
当要点是“X 调用 Y 调用 Z，并查看混乱情况”时，请使用美人鱼“流程图”或“图表”。将其包裹在 Tailwind 风格的卡片中，这样就不会感觉像降落伞一样。使用 classDef 进行样式，将泄漏边缘着色为红色，将深层模块着色为黑色。序列图适用于“之前：6 次往返；之后：1 次”。
```html
...
```
### 手工制作的盒子和箭头（当美人鱼的布局与你战斗时）

模块为带有边框和标签的“<div>”。作为内联 SVG `<line>` 或 `<path>` 元素的箭头绝对定位在相对容器上。当您希望“之后”图感觉像一个具有灰色内部结构的厚边框深层模块时，可以使用此选项 - Mermaid 不会以正确的权重渲染它。
...
简单的英语，简洁 - 但建筑名词和动词直接来自“/codebase-design-zh”技能。简洁并不是随波逐流的借口。
...
没有对冲，没有清嗓子，没有“值得注意的是……”。如果一句话可以成为一颗子弹，那就让它成为一颗子弹。如果可以切子弹，那就切它。如果某个术语不在“/codebase-design-zh”词汇表中，请在发明新术语之前先找到该术语。