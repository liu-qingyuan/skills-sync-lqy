...
此存储库的问题和 PRD 作为 GitLab 问题存在。使用 [`glab`](https://gitlab.com/gitlab-org/cli) CLI 进行所有操作。
...
从 `git remote -v` 推断存储库 — `glab` 在克隆内运行时会自动执行此操作。

## 合并请求作为分类表面

**MR 作为请求表面：否。** _（如果此存储库将外部合并请求视为功能请求，则设置为“yes”；“/triage-zh”读取此标志。）_

当设置为“yes”时，MR 会使用“glab mr”等效项运行与问题相同的标签和状态：

- **阅读 MR**：`glab mr view <number> --comments` 和 `glab mr diff <number>` 进行比较。
- **列出外部 MR 进行分类**：`glab mr list -F json`，然后仅保留作者不是项目成员/所有者的 MR（贡献者的 MR，而不是维护者的正在进行的工作）。
