...
此存储库的问题和 PRD 作为 GitHub 问题存在。使用 `gh` CLI 进行所有操作。
...
从 `git remote -v` 推断存储库 — `gh` 在克隆内运行时会自动执行此操作。

## 将请求作为分类表面

**PR 作为请求表面：否。** _（如果此存储库将外部 PR 视为功能请求，则设置为“yes”；“/triage-zh”读取此标志。）_

当设置为“yes”时，PR 会使用与问题相同的标签和状态，使用“gh pr”等效项：

- **阅读 PR**：“gh pr view <number> --comments”和“gh pr diff <number>”以获取差异。
- **列出外部 PR 进行分类**：`gh pr list --state open --json number,title,body,labels,author,authorAssociation,comments`，然后仅保留`CONTRIBUTOR`、`FIRST_TIME_CONTRIBUTOR`或`NONE`的`authorAssociation`（删除`OWNER`/`MEMBER`/`COLLABORATOR`）。
