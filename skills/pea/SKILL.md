---
name: pea
description: PEA，Project Engineering Architect。用于项目工程架构、模块设计、接口设计、重构、C4 架构规划、类图实现视图、事件/服务契约、依赖边界、策略治理和 CI/CD 质量门禁。按 Module、Boundary、Contract、Governance 四类架构职责判断，目标是低耦合、高内聚、深模块、可测试、可维护、不过度工程。
---
...
4. **Design**：做最小架构调整：先删除/合并浅模块，其次加深已有模块，最后才新增抽象；涉及架构变更时先更新 C4 图和类图，再改代码。
5. **Verify**：运行或说明测试、类型检查、依赖边界检查、契约检查、架构图一致性检查和 CI gate。


## 架构图和实现图

- 总体架构规划优先调用 `c4-architecture` skill；PEA 负责判断边界、职责、契约和治理规则。
- 先读取项目已有 `docs/architecture/` 作为当前标准；不要凭空覆盖已有架构约定。
- C4 图表达系统视角：Context、Container、Component、Deployment；复杂请求链路可补 Dynamic 图。
- 类图表达代码实现视角：关键 class、interface、type、继承/实现/组合关系和核心依赖方向。
- 架构修改顺序：先更新相关 C4 图和类图，再按图修改代码，最后验证图、代码、测试是否一致。
- 默认路径：C4 图放 `docs/architecture/c4-*.md`；类图放 `docs/architecture/class-{module-or-feature}.md`。
- 不要为了画图而画图；只有边界、职责、契约或实现关系会影响修改时才补图。
...
| Architecture docs | C4 图、类图、代码、测试一致 | 架构图过期或代码绕过图上的边界 |
