---
name: pea
description: PEA，Project Engineering Architect。用于项目工程架构、模块设计、接口设计、重构、C4 架构规划、Mermaid 架构图、类图和时序图实现视图、事件/服务契约、依赖边界、策略治理和 CI/CD 质量门禁。按 Module、Boundary、Contract、Governance 四类架构职责判断，目标是低耦合、高内聚、深模块、可测试、可维护、不过度工程。
---
...
4. **Design**：做最小架构调整：先删除/合并浅模块，其次加深已有模块，最后才新增抽象；涉及架构变更时先更新 C4 图、Mermaid 架构图、类图和时序图，再改代码。
5. **Verify**：运行或说明测试、类型检查、依赖边界检查、契约检查、架构图/类图/时序图一致性检查和 CI gate。


## 架构图、类图和时序图

- 总体系统架构规划优先调用 `c4-architecture` skill；PEA 负责判断边界、职责、契约和治理规则。
- 补充 Mermaid 架构图、类图、时序图优先调用 `mermaid-visualizer` skill；不要在 PEA 里复制完整画图规则。
- 先读取项目已有 `docs/architecture/` 作为当前标准；不要凭空覆盖已有架构约定。
- C4 图表达系统视角：Context、Container、Component、Deployment；复杂请求链路可补 Dynamic 图。
- Mermaid 架构图表达局部结构、流程或边界；类图表达关键 class、interface、type、继承/实现/组合关系和核心依赖方向；时序图表达跨模块、API、event、tool 或 provider 的调用顺序。
- 图里要体现 PEA 四类职责：Module=模块/接口/深度；Boundary=层级/seam/adapter/依赖方向；Contract=API/event/tool/provider contract；Governance=架构规则、policy、CI gate 或依赖边界检查。
- 架构修改顺序：先更新相关 C4 图、Mermaid 架构图、类图和时序图，再按图修改代码，最后验证图、代码、测试是否一致。
- 默认路径：C4 图放 `docs/architecture/c4-*.md`；类图放 `docs/architecture/class-{module-or-feature}.md`；时序图放 `docs/architecture/sequence-{flow}.md`；补充架构图放 `docs/architecture/architecture-{topic}.md`。
...
| 职责 | 图中必须看见什么 |
| --- | --- |
| Module | 关键模块、公开 Interface、隐藏的 Implementation、模块深度和主要调用方。 |
| Boundary | domain/application/infrastructure 边界，seam、adapter、provider/transport/storage/tool/event 边缘和依赖方向。 |
| Contract | API/event/tool/provider contract 的名称、payload/schema、版本、producer、consumer。 |
| Governance | 依赖规则、循环依赖禁止、policy-as-code、CI gate、architecture check 的落点。 |
...
| Architecture docs | C4 图、Mermaid 架构图、类图、时序图、代码、测试一致 | 图过期或代码绕过图上的边界 |
...
- 不要在 C4 图、Mermaid 架构图、类图或时序图未同步时声称架构修改完成。
