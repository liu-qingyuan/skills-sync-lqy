...
| 阶段 | 能力 | PEA 关注点 |
| --- | --- | --- |
| 过程式编程 | 写功能 | 行为正确，避免散乱副作用。 |
| 面向对象编程 | 组织代码 | 对象职责、封装和内聚。 |
| 接口抽象 / 依赖倒置 | 解耦模块 | Interface、Seam、Adapter、依赖方向。 |
| 领域驱动设计 | 划分业务边界 | 领域模型、聚合、用例、业务规则归属。 |
| 事件驱动架构 | 设计系统协作 | 事件模型、异步边界、生产者/消费者、幂等。 |
| 策略治理 / 可观测 / 可演进架构 | 治理系统演进 | 契约模型、policy、CI gate、观测、兼容性。 |

## 三个核心模型

| 模型 | PEA 定义什么 | TEA 对应保护 |
| --- | --- | --- |
| 领域模型 DDD | 聚合、实体、值对象、领域服务、用例边界和业务规则归属。 | unit/integration 验证业务不变量、状态变化和用例结果。 |
| 事件模型 | 事件名、payload、版本、生产者/消费者、顺序、幂等和兼容性。 | schema/contract/integration 验证事件兼容和处理行为。 |
| 契约模型 | API/event/tool/provider/CLI 的输入输出、错误、schema/type 和版本。 | contract/integration 验证实现符合契约定义。 |
