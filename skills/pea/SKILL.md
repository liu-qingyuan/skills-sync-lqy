---
name: pea
description: PEA，Project Engineering Architect。用于项目工程架构、模块设计、接口设计、重构、C4 宏观到微观架构规划、Mermaid 类图和时序图实现视图、事件/服务契约、依赖边界、策略治理和 CI/CD 质量门禁。按 Module、Boundary、Contract、Governance 四类架构职责判断，目标是低耦合、高内聚、深模块、可测试、可维护、不过度工程。
---
