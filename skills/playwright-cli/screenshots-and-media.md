...
### Electron App Video Recording

For Playwright Test projects that launch Electron with `_electron.launch()`, config-level `use.video = 'on'` may not create `.webm` files because the recorded page is created by a custom Electron application fixture, not by the normal browser project context. Prefer a fixture switch that passes `recordVideo` directly into `_electron.launch()`:

```ts
const recordVideoDir = process.env.PLAYWRIGHT_ELECTRON_RECORD_VIDEO_DIR?.trim();
...
Run the Electron E2E test with an explicit artifact directory:

```bash
PLAYWRIGHT_ELECTRON_RECORD_VIDEO_DIR="$PWD/test-results/electron-videos" \
  npx playwright test --project=electron --headed
```

After the Electron app closes, inspect that directory for `.webm` files. If no video appears, confirm the fixture really forwards `recordVideo`, the target window/page is created after launch, and the app/browser context closes cleanly. Keep trace and screenshots enabled as fallback evidence.
