...
- Workspace provision：`scripts/provision_workspace.py`，供 `to-spec-lqy`、`to-tickets-lqy` 和 `triage-lqy` 按 exact branch 创建或复用 worktree，linked worktree 继承共享 Git config/ignore rules，默认严格检查 base drift
