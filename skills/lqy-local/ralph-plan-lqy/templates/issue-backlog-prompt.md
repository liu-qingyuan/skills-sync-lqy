...
每个 Ticket 只允许一次 broad review（2 个只读 agents，`max_turns: 6`）和最多一次 focused closure（resume 同一 IDs，`max_turns: 3`）；review Agent 调用上限为 4，失败也计入。reviewer 禁止 GitNexus、index/graph 查询、嵌套 `Agent`、替代 reviewer、第三轮或“最终 review”。

只有具备明确证据的 `Critical` / `High` findings 阻塞。修复必须是满足原验收的最小改动；`Medium`、`Low`、heuristic 和 optional hardening 写 follow-up，不继续调查。closure 只验证原 findings，不重扫完整 diff。blocking findings 关闭且门禁通过后立即 commit/push/close。

实现超出一个新上下文或 broad review 暴露大量跨模块 findings 时，提交绿色完整增量并停止；评论拆分建议，保持 issue open，将 `ready-for-agent` 替换为 `needs-triage`。不要用更多 reviewer、harness 或抽象强行收敛。

# 铁律

- 一次只做一个 issue。
- `PI_RUN_RALPH_WORKER=1` 表示当前进程已经是 Ralph iteration；禁止调用 `run_ralph` 或 CLI 启动嵌套 Ralph。
