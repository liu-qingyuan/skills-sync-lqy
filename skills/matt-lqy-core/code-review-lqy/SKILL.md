...
如果当前环境支持并行 sub-agent，分别启动 Standards 和 Spec 两个通用 sub-agent。优先使用 Codex 可用的 agent 能力；不要因为仓库里存在 `CLAUDE.md` 就默认选择 Claude。若当前环境没有 sub-agent，就分开执行两个检查，并保持两个轴的发现互不合并。
