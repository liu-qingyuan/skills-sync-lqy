...
5. 在向用户给出最终方案或执行任何发布前，必须对完整 spec 草稿做一次设计终审，并按结果直接修订草稿：

   - 用 `$codebase-design-lqy` 检查 Module、Interface、Seam 和 Depth，优先复用已有接缝，删除浅模块和不必要的 Adapter。
   - 用 `$gitnexus` 检查索引状态；可用时查看相关代码的 context 和 impact，并用源码与测试确认关键结论。索引缺失或过期时先建立或刷新；工具不可用时回退到源码检查并说明。
   - 用 `$simple` 删除重复决策、过早抽象和不必要流程，同时保留需求、契约和安全约束。
