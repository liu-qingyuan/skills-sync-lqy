...
6. GitHub Ralph-ready 父 spec 必须通过本 skill 的 publisher 发布。先把完整正文写入 body file，再运行：
   ...
   publisher 先检查模板 section 完整、有序且非空（包括 `## Mermaid Gate`），再用共享 contract validator 检查完整正文，然后创建**不带** `ready-for-agent` 的 issue。它会回读并重复两层验证，最后才应用 `ready-for-agent`。任何失败都停止；已经创建的 draft issue 保持无 ready label，不要绕过验证补标签。
