...
6. GitHub Ralph-ready 父 spec 必须通过本 skill 的 publisher 发布。先把完整正文写入 body file，再运行：
   ...
   publisher 先用共享 contract validator 检查完整正文，再创建**不带** `ready-for-agent` 的 issue，回读并再次验证正文，最后才应用 `ready-for-agent`。任何失败都停止；已经创建的 draft issue 保持无 ready label，不要绕过验证补标签。
