...
> 说明：“issue tracker”是此仓库实际跟踪工作的地方。`to-issues`、`triage`、`to-spec` 和 `qa` 等 skill 会读取和写入它，所以它们需要知道应该调用 `gh issue create`、在 `.scratch/` 下写 Markdown 文件，还是遵循你描述的其他工作流。请选择你真实使用的工作跟踪位置。
