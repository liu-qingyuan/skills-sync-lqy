...
**map、child Ticket、blocking 和 frontier query 的物理表达取决于 tracker。** issue tracker 应该已经提供；如果没有，请运行 `/setup-matt-pocock-skills-lqy`。查看 tracker 文档中的 `Wayfinding operations` 部分，了解此仓库如何表达它们。如果没有提供 tracker，默认使用 local-markdown tracker。

map、Ticket、issue comment 和 resolution 默认使用中文。labels、命令、路径、代码标识符、配置键和错误原文保留原 token。
...
- **Prototype** (HITL)：通过制作便宜、粗糙、具体的 artifact 来提高讨论保真度，例如 outline、rough take、stub，或通过 `/prototype-lqy` 创建 UI/logic code。把 prototype 作为 asset 链接。关键问题是“它应该看起来如何”或“它应该如何表现”时使用。
- **Grilling** (HITL)：通过 `/grilling-lqy` 和 `/domain-modeling-lqy` skills 进行对话，一次一个问题。默认类型。
...
1. **Name the destination.** 运行 `/grilling-lqy` 和 `/domain-modeling-lqy` 会话，钉住这张 map 正在寻找路线通向什么：spec、decision 或 change。destination 固定 scope，所以它先确定。
...
3. Resolve it：**按需 zoom**。只在需要时获取相关或已关闭 Ticket 的完整 body；调用 `## Notes` block 指定的 skills。不确定时，使用 `/grilling-lqy` 和 `/domain-modeling-lqy`。
