...
3. 运行安装版 `triage-lqy` 自带的 publisher。用户未指定 branch/base branch 时不要传对应参数；共享 resolver 会使用远程默认 branch，并固定 fetch 后的完整 base commit：
...
4. publisher 返回非零时停止。issue 可能已经保留已验证的正文或 brief，但不得手工补 `ready-for-agent`。退出码 `3` 表示持久契约或 publication gate 错误，退出码 `1` 表示 GitHub、git、I/O 或环境失败。
5. 只有用户已经看到 base drift 的旧 SHA、新 SHA 和提交摘要，并明确选择继续使用旧 SHA 时，才可从头重跑并追加 `--allow-base-drift`。不要默认绕过 drift gate。
