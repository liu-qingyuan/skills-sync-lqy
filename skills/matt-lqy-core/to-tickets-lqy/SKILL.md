...
- 按依赖顺序创建全部无 ready label 的 issues，统一生成 `## Parent`、`## Blocked by` 和最终 `## Git`。
- 回读并验证整组标题、正文、open 状态、Mermaid Gate 和 Git 契约；全部通过后才按反向依赖顺序应用 `ready-for-agent`。标签 API 失败时撤销本组已应用的 ready labels。

如果 provisioner 报告 `base drift`，停止并向用户展示记录的旧 SHA、远程新 SHA 和相关提交摘要，等待明确选择。选择保留旧 SHA 表示本次不发布并保留父 spec；选择新 SHA 时先由用户明确批准更新父 spec 的 `Base commit`，然后从头重新验证和运行 publisher。不要静默刷新父 spec，也不要绕过 drift gate。

部分创建、回读或标签失败时，已经创建的 draft issues 保持无 `ready-for-agent`；不要手工补标签或关闭/修改父 spec。修复原因后重新规划如何处理这些 draft issues，避免重复发布。
