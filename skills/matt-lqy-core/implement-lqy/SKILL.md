...
如果需要图，先使用 /mermaid-visualizer，并把图放到 issue 正文、issue comment 或 PR 描述中，再开始实现。

默认只画**一张**最能解除实现歧义的图；不要因为 issue 同时提到多个模块、接口或调用关系，就机械画多张图。多个对象之间的接口和调用关系通常合并到同一张架构/调用关系图中。只有 issue 明确要求多个视角，或一张图无法清楚表达不同问题时，才补充第二张图。

选图顺序：

- 架构/调用关系图：默认首选；用于模块边界、接口、adapter、跨模块依赖和“谁调用谁”。
- 时序图：用于解释一次请求、一次工具调用或一次业务流程的时间顺序。
- 状态图：用于状态机、生命周期、任务状态或可回退流程。
- 类图：仅用于局部类/接口结构，避免画全项目类图。
