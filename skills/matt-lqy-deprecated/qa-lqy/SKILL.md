...
不要过度追问。如果描述足够清晰，可以归档，请继续。

### 2.在后台探索代码库

与用户交谈时，在后台启动 agent（`subagent_type=Explore`） 以了解相关区域。我们的目标不是找到解决办法，而是：
...
issue 必须是**持久的**——即使经过重大重构，内容仍然应该有价值。请从用户视角来写。
...
- **最大化并行性** - 目标是多个人（或 agent）可以同时处理不同的 issue
