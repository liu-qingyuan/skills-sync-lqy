---
name: handoff
description: Generate a compact copy-paste handoff prompt for another AI or fresh session. Use when the user invokes $handoff or asks to hand off, delegate, transfer context, or prepare a prompt for another agent. Do not write files and do not execute the handed-off task.
---
...
- 普通 `$handoff` 不追问；把不确定性转成接手 AI 的确认/停止边界。
- 实现细节默认交给接手 AI 判断。
...
修改后至少运行 `quick_validate.py`。未做真实 `$handoff ...` smoke 时，不要声称运行时行为已验证。
