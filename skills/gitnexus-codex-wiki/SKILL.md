...
Use `interactive-flow` when the architecture graph is dense enough that Mermaid/SVG would create overlapping labels, ambiguous arrows, or weak inspection affordances. For full-coverage architecture-web deliveries, `interactive-flow` is the primary renderer page-wide: `index.html` must carry the required overview/runtime/flow/branch graphs, and every module page must carry at least one primary React Flow graph. Simple tree, checklist, or audit diagrams may remain Mermaid/SVG only when they are explicitly non-primary or inside collapsed source/audit details.
...
- Mark each delivered React Flow mount with `data-flow-graph="<graph-id>"` and pair it with a page-local `<script type="application/json" data-architecture-flow>` payload whose `graphs[].id` values cover every mounted graph. Do not rely on asset existence or an audit JSON file as proof of delivered page coverage.
...
- Do not ship visible primary Mermaid/`graph TB`/`flowchart` text or visible static SVG diagrams when React Flow full coverage is required. Retained Mermaid/SVG/source is allowed only as collapsed audit/source material or explicit non-primary evidence/provenance.
