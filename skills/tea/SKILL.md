...
| Contract 契约测试 | API、event、tool protocol、provider、schema、service contract；Pact/PactFlow 可用于验证兼容性。 | 不负责定义默认契约格式，不验证内部实现和 UI 流程。 |
...
- Contract test 应保护 payload 形状、版本、兼容性和错误行为；Pact/PactFlow 可用于验证 consumer/provider 兼容性。
...
- Pact 不是默认契约定义格式；Pact 是可选的 consumer/provider 兼容性测试工具。
...
- 如果已有 OpenAPI/AsyncAPI/schema contract 也要进入 Pact 风格 CI gate，考虑 PactFlow/Swagger Contract Testing 的 bi-directional contract testing。
...
- 需要 Pact/PactFlow consumer test、provider verification、Pact Broker/PactFlow、can-i-deploy 配置或诊断时，使用 `pactflow`；但契约定义仍以项目已有 contract 文件为准。
