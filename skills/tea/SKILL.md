...
| 模型 | TEA 保护什么 |
| --- | --- |
| 领域模型 DDD | 业务不变量、状态变化、领域规则和用例结果；优先 unit/integration。 |
| 事件模型 | 事件名、payload、版本、生产者/消费者、顺序、幂等和兼容性；优先 schema/contract/integration。 |
| 契约模型 | API/event/tool/provider/CLI 的输入输出、错误、schema/type 和版本；优先 contract/integration。 |
