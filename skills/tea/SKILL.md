...
| Contract 契约测试 | API、event、tool protocol、provider、schema、service contract；Pact 可用于验证兼容性。 | 不负责定义默认契约格式，不验证内部实现和 UI 流程。 |
...
- Contract test 应保护 payload 形状、版本、兼容性和错误行为；默认用开源 Pact 验证 consumer/provider 兼容性。
- 需要生成 Pact consumer test、provider verification、自建 Pact Broker、can-i-deploy 配置或诊断时，使用 `pact` skill；TEA 只负责判断这里需要 Contract Test。
- 架构检查应在技术栈支持时强制依赖方向。
...
- 如果已有 OpenAPI/AsyncAPI/schema contract 也要进入 Pact 风格 CI gate，默认先评估开源 Pact + 自建 Pact Broker。
...
- 需要 Pact consumer test、provider verification、自建 Pact Broker、can-i-deploy 配置或诊断时，使用 `pact`；契约定义仍以项目已有 contract 文件为准。
