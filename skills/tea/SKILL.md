...
| Contract 契约测试 | API、event、tool protocol、provider、schema、service contract；Pact 属于这一层。 | 不验证内部实现和 UI 流程。 |
...
- Contract test 应保护 payload 形状、版本、兼容性和错误行为；Pact 可用于 consumer/provider contract。
- 架构检查应在技术栈支持时强制依赖方向。
