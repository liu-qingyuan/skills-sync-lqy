...
- Contract test 应保护 payload 形状、状态码/事件名、版本、兼容性和错误行为。
...
- Contract testing 是验证实现是否符合契约；可以用 schema validation、API/integration test、provider verification、consumer-driven Pact，或项目已有工具。
- Pact 不是默认契约格式，也不是契约层必选项；它只在需要保护 consumer 真实依赖、consumer/provider 独立开发/版本/发布时使用。
- 如果 YAML/schema/typed contract + integration/API/E2E 已能覆盖风险，不要引入 Pact。
