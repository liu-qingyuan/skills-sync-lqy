...
| PEA 模型 | TEA 保护什么 | 常用测试层 |
| --- | --- | --- |
| 领域模型 DDD | 业务不变量、状态变化、领域规则和用例结果。 | unit / integration |
| 事件模型 | 事件名、payload、版本、生产者/消费者、顺序、幂等和兼容性。 | schema / contract / integration |
| 契约模型 | API/event/tool/provider/CLI 的输入输出、错误、schema/type 和版本。 | contract / integration / API or CLI test |
