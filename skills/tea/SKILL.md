...
2. **Choose level**：选择 unit、integration、contract、E2E、architecture check 或 smoke。
...
| 层级 | 适用场景 |
| --- | --- |
| Unit | 纯领域逻辑和稳定小 Interface。 |
| Integration | 模块协作、真实配置、真实依赖或接近真实的 fake。 |
| Contract | API、event、tool protocol、provider、schema、service contract。 |
| E2E | 关键用户路径或端到端业务链路。 |
| Architecture check | 依赖方向、循环依赖、层级边界、禁止 import。 |
| Smoke | 部署、打包或集成后的关键路径检查。 |

## 测试应该服务架构

- 业务测试不应理解 provider、transport、storage、framework 或 tool 内部细节。
- Adapter 测试应证明外部 contract 与内部模型之间的转换。
- Contract test 应保护 payload 形状、版本、兼容性和错误行为。
- 架构检查应在技术栈支持时强制依赖方向。
...
- static analysis、architecture check、lint/typecheck 是测试架构的一层，用来自动发现可规则化的问题。
