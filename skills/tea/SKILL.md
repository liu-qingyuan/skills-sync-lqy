...
| Contract 契约测试 | API、event、tool protocol、provider、schema、service contract；Pact 可用于验证兼容性；PactFlow 不是默认依赖。 | 不负责定义默认契约格式，不验证内部实现和 UI 流程。 |
...
- Contract test 应保护 payload 形状、版本、兼容性和错误行为；默认用开源 Pact 验证 consumer/provider 兼容性；PactFlow 只在项目已有或用户明确要求时使用。
- 需要生成 Pact consumer test、provider verification、自建 Pact Broker、can-i-deploy 配置或诊断时，使用 `pactflow` skill；TEA 只负责判断这里需要 Contract Test。
- 架构检查应在技术栈支持时强制依赖方向。
- 回归测试应先能复现已观察到的 bug，再通过真实公开表面验证修复。

## Contract definition vs Contract testing

- 契约定义沿用项目已有权威格式：OpenAPI/AsyncAPI YAML、JSON Schema、schema YAML、typed contract、Pact JSON 之外的自定义 JSON，或项目已有的自定义 contract 文件。
- 不要因为要做契约测试就替换已有契约定义格式；除非用户明确要求迁移。
- Pact 不是默认契约定义格式；Pact 是可选的 consumer/provider 兼容性测试工具。默认免费方案是 Pact + 自建 Pact Broker。
- 默认规则：已有 contract format 继续定义契约，Pact 只负责验证兼容性。
- Classic Pact：consumer test 生成 Pact JSON，provider verification 用这些生成物验证真实 provider。
- 不要默认手写 Pact JSON/YAML，也不要默认用 Pact 替代已有 YAML/JSON/schema/typed contract。
- 如果已有 OpenAPI/AsyncAPI/schema contract 也要进入 Pact 风格 CI gate，默认先评估开源 Pact + 自建 Pact Broker；只有项目已有 PactFlow 或用户明确要求商业/SaaS/bi-directional contract testing 时，才考虑 PactFlow/Swagger Contract Testing。
...
- 需要 Pact consumer test、provider verification、自建 Pact Broker、can-i-deploy 配置或诊断时，使用 `pactflow`；PactFlow 商业/SaaS 能力只在项目已有或用户明确要求时使用；契约定义仍以项目已有 contract 文件为准。
